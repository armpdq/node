...
describe('queries', () => {
	...
	it('insert into select', async () => {
		...
		await clickhouse.query('DROP TABLE IF EXISTS source_table').toPromise();
		await clickhouse.query('DROP TABLE IF EXISTS dist_table').toPromise();
		await clickhouse.query('CREATE TABLE source_table (id UInt32) ENGINE=MergeTree PARTITION BY tuple() ORDER BY tuple()').toPromise();
		await clickhouse.query('CREATE TABLE dist_table (id UInt32) ENGINE=MergeTree PARTITION BY tuple() ORDER BY tuple()').toPromise();

		await clickhouse.query(
			'INSERT INTO source_table SELECT number FROM system.numbers LIMIT 1000000'
		).toPromise();
